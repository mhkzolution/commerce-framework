...
final class ProductWorkspaceApiTest extends TestCase
{
    ...
    {
        $variant = $this->createPurchasableProduct(price: 45, sku: 'API-WS-001');
        $product = $variant->product;

        $this->actingAs(User::query()->first())
            ->getJson(route('api.v1.admin.products.workspace.show', $product))
            ->assertOk()
    ...
    {
        $variant = $this->createPurchasableProduct(price: 30, sku: 'API-UPD-001');
        $product = $variant->product;

        $this->actingAs(User::query()->first())
            ->putJson(route('api.v1.admin.products.workspace.update', $product), [
        ...
        $this->assertSame('55.00', (string) $variant->price);
    }
}
