...
final class ProductCsvImportTest extends TestCase
{
    ...
    public function test_admin_can_create_product_from_csv(): void
    {
        ...
        $this->assertSame(199.0, (float) $variant->price);
        $this->assertSame(299.0, (float) $variant->compare_at_price);
    ...
    public function test_admin_can_export_product_with_seller_column(): void
    {
        ...
        $variant = $this->createPurchasableProduct(price: 120, stock: 4, sku: 'CSV-SELLER-EXP');
    ...
    {
        $variant = $this->createPurchasableProduct(price: 100, stock: 5, sku: 'CSV-UPD-001');
        ...
        $this->assertSame(150.0, (float) $variant->price);
    ...
    {
        $variant = $this->createPurchasableProduct(price: 150, stock: 3, sku: 'CSV-EXP-001');
        $product = $variant->product;
        $product->update([
    ...
    public function test_admin_edit_page_uses_create_layout_sections(): void
    {
        $variant = $this->createPurchasableProduct(price: 10, stock: 1, sku: 'LAYOUT-001');
        $product = $variant->product;
