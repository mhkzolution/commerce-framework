...
final class StorefrontAuthRedirectTest extends TestCase
{
    ...
    public function test_query_redirect_becomes_intended_url(): void
    {
        $request = Request::create('https://shop.test/account/login', 'GET', [
            'redirect' => 'https://shop.test/checkout',
        ]);
        $request->setLaravelSession(app('session.store'));

        StorefrontAuthRedirect::rememberFromRequest($request);

        $this->assertSame('https://shop.test/checkout', $request->session()->get('url.intended'));
    }
}
