...
@section('page')
    <x-admin.page title="Edit Brand">
        ...
        <x-admin.form.shell action="{{ route('admin.catalog.brands.update', $brand) }}" method="POST" class="max-w-2xl">
            @csrf
            @method('PUT')
            <x-admin.form.section title="Brand details">
                @include('catalog::admin.brands._form', ['brand' => $brand])
