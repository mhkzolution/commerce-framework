...
final class ProductWorkspaceSaveService
{
    ...
    private function persistProduct(?Product $existing, SaveProductWorkspaceData $data): Product
    {
        ...
        $this->syncAutomatedCollections($product);
    ...
    private function syncVariants(Product $product, array $variants): void
    {
        ...
        foreach (array_values($variants) as $position => $row) {
            ...
            $attributes = [
                ...
                'price' => ProductPrice::normalize($row['price'] ?? 0),
                'compare_at_price' => self::nullablePrice($row['comparePrice'] ?? null),
                'cost' => self::nullablePrice($row['cost'] ?? null),
                'weight' => self::nullablePrice($row['weight'] ?? null),
                'status' => $status,
    ...
    private function syncRelations(Product $product, array $categoryIds, array $collectionIds, array $tagIds, array $mediaUuids): void
    {
        ...
        $manualCollectionIds = Collection::query()
            ->where('type', Collection::TYPE_MANUAL)
            ->whereIn('id', $collectionIds)
            ->pluck('id')
            ->all();
        $product->collections()->sync($manualCollectionIds);
    ...
    private static function nullablePrice(mixed $value): ?float
    {
        if ($value === null || $value === '') {
            return null;
        }

        return ProductPrice::normalize($value);
    }
    ...
    private function syncAutomatedCollections(Product $product): void
    {
        if (! app()->bound(CollectionAutomatedSyncService::class)) {
            return;
        }

        app(CollectionAutomatedSyncService::class)->syncForProduct($product);
    }
}
