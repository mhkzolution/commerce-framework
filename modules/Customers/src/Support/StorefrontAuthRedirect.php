...
final class StorefrontAuthRedirect
{
    public static function rememberFromRequest(Request $request): void
    {
        ...
        if (is_string($redirect) && self::isAllowedStorefrontUrl($redirect, $request)) {
    ...
    public static function rememberUrl(string $url, ?Request $request = null): void
    {
        if (self::isAllowedStorefrontUrl($url, $request)) {
