...
final class FooterController extends Controller
{
    public function __construct(
        ...
        private readonly ?StorefrontNavigationConfig $navigationConfig = null,
        private readonly ?SiteIdentityService $siteIdentity = null,
    ) {}
    ...
    {
        $items = is_array($this->navigationConfig?->resolve()['items'] ?? null)
            ? $this->navigationConfig->resolve()['items']
            : [];
    ...
    private function socialSources(): array
    {
        ...
        foreach ($this->siteIdentity?->socialLinks() ?? [] as $link) {
            if (! is_array($link) || ! is_string($link['key'] ?? null)) {
                continue;
            }

            $configured[] = [
                'key' => $link['key'],
                'label' => is_string($link['label'] ?? null) ? $link['label'] : Str::headline($link['key']),
                'url' => $link['url'] ?? null,
            ];
        }
        ...
        $configuredKeys = array_fill_keys(array_column($configured, 'key'), true);

        return [
            'configured' => $configured,
            'missing' => array_values(array_filter(
                $all,
                static fn (array $item): bool => ! isset($configuredKeys[$item['key']]),
            )),
        ];
    }
}
