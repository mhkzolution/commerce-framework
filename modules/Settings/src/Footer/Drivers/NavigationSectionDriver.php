...
final class NavigationSectionDriver implements FooterSectionDriver
{
    ...
    {
        try {
            if (! $config->enabled || ! $this->isModuleEnabled('cart')) {
                return null;
            }
            ...
            return new FooterSection(
                id: $config->id,
                type: $config->type,
                titleKey: 'footer.section.navigation',
                items: $items,
                meta: [
                    'source' => $source,
                    'count' => count($items),
    ...
    private function resolveItems(
        ...
        if ($source !== 'main' || $this->navigationConfig === null) {
            return [];
        }

        $resolved = $this->navigationConfig->resolve();
        $items = is_array($resolved['items'] ?? null) ? $resolved['items'] : [];

        return $this->normalizeItems($items, $maxLinks, $visibilityMode);
    }
    ...
    private function isModuleEnabled(string $alias): bool
    {
        return $this->modules?->isEnabled($alias) ?? true;
    }
