...
final class PosStateService
{
    ...
    public function searchProducts(string $query, int $limit = 20): array
    {
        ...
        return array_map(function (ProductVariant $variant) use ($stockLevels, $imageMap, $currency): array {
            ...
            return [
                ...
                'sku' => $variant->sku,
                'stock' => $available,
                'price' => $this->formatMoney(ProductPrice::toMinorUnits($variant->price), $currency),
                'price_minor' => ProductPrice::toMinorUnits($variant->price),
    ...
    private function formatMoney(int $minor, string $currency = 'THB'): string
    {
        ...
        return $this->currencySymbol($currency).number_format(ProductPrice::fromMinorUnits($minor), 2);
    }
