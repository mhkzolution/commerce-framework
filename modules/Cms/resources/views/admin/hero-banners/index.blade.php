...
@section('title', __('cms::admin.hero_banners'))
@section('page')
    <x-admin.page :title="__('cms::admin.hero_banners')" :description="__('cms::admin.hero_banners_description')">
        <x-slot:primaryActions>
            <x-admin.button variant="primary" :href="route('admin.cms.hero-banners.create')">{{ __('cms::admin.create_hero_banner') }}</x-admin.button>
        </x-slot:primaryActions>
        <x-admin.table.shell>
            <x-slot:head>
                <tr>
                ...
                <tr>
                    <td class="px-4 py-3">
                        @include('cms::admin.partials.media-thumb', [
                            ...
                            'alt' => __('cms::admin.hero_type_'.$item->type),
                        ])
                    </td>
                    <td class="px-4 py-3 font-medium text-text">{{ __('cms::admin.hero_type_'.$item->type) }}</td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-right"><x-admin.button variant="link" :href="route('admin.cms.hero-banners.edit', $item)">{{ __('cms::admin.edit') }}</x-admin.button></td>
                </tr>
            @empty
